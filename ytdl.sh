...
download_playlists() {
  ...
  # --print "%(id)s\t%(title)s" on /playlists gives real PLxxxx IDs + titles
  # Each entry is one playlist. Use --no-flat-playlist to get playlist entries.
  local PLAYLISTS
  PLAYLISTS=$(yt-dlp \
    --flat-playlist \
    --no-warnings \
  ...
  while IFS=$'\t' read -r pl_id pl_title; do
    [[ -z "$pl_id" ]] && continue
    # Skip if not a real playlist ID (must start with PL)
    [[ "$pl_id" != PL* ]] && continue
    count=$((count+1))
