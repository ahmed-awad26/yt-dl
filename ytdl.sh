...
# Resolve channel URL if input is a video/playlist link
RESOLVED_URL=$(yt-dlp --skip-download --no-warnings --print "%(channel_url)s" "$RAW_URL" 2>/dev/null | head -1)
if [[ -z "$RESOLVED_URL" || "$RESOLVED_URL" == "NA" || "$RESOLVED_URL" == "None" ]]; then
  RESOLVED_URL=$(yt-dlp --skip-download --no-warnings --print "%(uploader_url)s" "$RAW_URL" 2>/dev/null | head -1)
fi
if [[ -n "$RESOLVED_URL" && "$RESOLVED_URL" != "NA" && "$RESOLVED_URL" != "None" ]]; then
  ...
  echo ""
  echo -e "  ${DIM}Fetching channel info...${RESET}"

  CHANNEL_NAME=""

  # Try multiple URL variants to get channel name
  ...
  # Last resort: extract name from URL itself
  ...
  if [[ -z "$CHANNEL_NAME" ]]; then
    ...
    warn_msg "Could not detect channel name -- using: $CHANNEL_NAME"
  else
